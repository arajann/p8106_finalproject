...
```{r}
pdp.gbm <- gbmA.fit %>%
            partial(pred.var = "stroke",
            grid.resolution = 100,
            prob = TRUE) %>%
            autoplot(rug = TRUE, train = trainData_pp) +
            ggtile("Boosting")
...
From the summary table we see that the model with the highest median and mean ROC is the adaptive boosting model. 


Extracting AUCs for each model 
```{r}
pred.svml <- predict(svml.fit, newdata = trainData_pp, type="prob")
pred.svmr <- predict(svmr.fit, newdata = trainData_pp, type="prob")
pred.mars <- predict(mars.fit, newdata = trainData_pp, type = "prob")[,1]
pred.glmn <- predict(model.glmn, newdata=trainData_pp, type= "prob")[,1]

roc.gbmA <- roc(trainData_pp$stroke, gbmA.pred)
roc.svml <- roc(trainData_pp$stroke,pred.svml[,2])
roc.svmr <- roc(trainData_pp$stroke,pred.svmr[,2])
roc.mars <- roc(trainData_pp$stroke,pred.mars)
roc.glmn <- roc(trainData_pp$stroke,pred.glmn)

modelNames <- c("Adaboost","Linear Kernel","Radial Kernel", "MARS","Penalized Logistic Regression")

ggroc(list(roc.gbmA,roc.svml,roc.svmr,roc.mars,roc.glmn),legacy.axes = TRUE) + scale_color_discrete(labels=paste0(modelNames,"(",round(auc,3),")"), name="Models(AUC)") +
  geom_abline(intercept=0,slope=1,color="grey")


auc <- c(roc.gbmA$auc[1], roc.svml$auc[1],roc.svmr$auc[1],roc.mars$auc[1], roc.glmn$auc[1])
...
=======
>>>>>>> e881afeaaf3dfae52839f622896e34ab3430e3c7
