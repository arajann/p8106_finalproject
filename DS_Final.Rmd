...
## Data Cleaning
To properly analyze the data and build models to make predictions, the first step is to clean the data. The vast majority of the data was already fairly clean and ready for analysis, but I began by reading in the data, cleaning variable names, making categorical data factors, making continuous data of type numeric, and dropping missing values. I also decided to exclude the variable "id" since it will not be relevant to my analysis. 
...
To begin the modeling process, I split the data into training (75%) and test (25%) data. 
```{r, echo=FALSE}
set.seed(1)
...
## Imputation
We apply the k nearest neighbor (k = 5) method for imputation where we assume 
any missing data is missing at random.
```{r}
...
trainData_pp <- bake(prep, new_data = trainData)
testData_pp <- bake(prep, new_data = testData)
```

The training data now contains `r nrow(trainData_pp)` rows.
...
The second model I fit to the dataset was Penalized Logistic Regression. 
```{r, echo=FALSE}
...
The tuning parameter in the penalty term of the model controls its flexibility. I decided to test a grid of tuning parameters. For alpha, since I wanted to use the elastic net method, I created a sequence from 0 to 1. For lambda, I created a sequence of numbers to test different values. The model then selects optimal values from the grid which are: alpha = `r model.glmn$bestTune$alpha` and lambda = `r model.glmn$bestTune$lambda`. To display this visually, I created a plot of the model AUC for various tuning parameters in the grid.
...
Let's train a MARS model on the imputed training data.
...
Let's examine the cross-validation plot.
```{r}
ggplot(mars.fit) +
  theme_bw() +
...
Let's get the values of degree (the degree of interaction) and nprune 
(the number of retained terms) that maximize the cross-validated AUC.
```{r}
mars.fit$bestTune %>% 
  as_tibble() %>% 
  knitr::kable()
...
Let's train a support vector machine with a radial kernel on the imputed 
training data.
```{r}
...
Let's get the values of sigma and cost that maximize the cross-validated
accuracy.
```{r}
svmr.fit$bestTune %>% 
  as_tibble() %>% 
...
Let's get the support vector machine with radial kernel that maximizes the 
...
For the next model, we fit was a support vector machine with a linear kernel on the imputed training data.
```{r}
set.seed(1)
...
SVM's with a linear kernel contain a tuning parameter C, also known as Cost, that determines the possible misclassifications. It essentially imposes a penalty to the model for making an error: the higher the value of C, the less likely it is that the SVM algorithm will misclassify a point. We decided to test a grid of tuning parameters for C. The model then selects the optimal value from the grid which maximizes the model AUC. That value is C = `r svml.fit$bestTune$C`. To display this visually, we created a plot of the model AUC for various values of the tuning parameter in the grid.
```{r}
plot(svml.fit, highlight = TRUE, xTrans = log)
```

The final SVM with a linear kernel that maximizes the cross-validated accuracy needed is displayed below.
```{r}
svml.fit$finalModel
```
...
```
```{r}
gbmA.pred <- predict(gbmA.fit, newdata = testData_pp, type = "prob")[,1]
